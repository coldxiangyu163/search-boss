...
async function main() {
  ...
  try {
    ...
    const migratedAdmins = await client.query(`
      update users
      set role = 'dept_admin'
      where role = 'enterprise_admin'
    `);
    console.log(`[migrate] migrated enterprise_admin users: ${migratedAdmins.rowCount}`);

    // Create default admin
    const adminHash = await bcrypt.hash('admin123', 10);
    const adminResult = await client.query(`
      insert into users (department_id, name, email, password_hash, role)
      values ($1, '默认管理员', 'admin@company.com', $2, 'dept_admin')
