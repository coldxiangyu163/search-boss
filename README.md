...
- `system_admin`：全局管理角色，可查看和维护全部部门、HR、BOSS 账号与浏览器实例。
- `dept_admin`：部门管理员，只能看到自己部门下的 HR、BOSS 账号、浏览器实例、职位、候选人与任务执行记录。
- `hr`：仅操作自己绑定的 HR 账号及其业务数据。

说明：

- `enterprise_admin` 已降级为**历史迁移角色**，当前后端不再允许创建或修改为该角色。
- 管理后台中的 `dept_admin` 文案统一按“本部门 / 部门范围”表达，避免误导为全局可见。
