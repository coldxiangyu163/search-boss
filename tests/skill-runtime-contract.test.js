...
const hasSkillWorkspace = fs.existsSync(skillRoot);
const skillTest = hasSkillWorkspace ? test : test.skip;

const bossSourcingSkill = hasSkillWorkspace
  ? fs.readFileSync(path.join(skillRoot, 'boss-sourcing', 'SKILL.md'), 'utf8')
  : '';
const runtimeContract = hasSkillWorkspace
  ? fs.readFileSync(path.join(skillRoot, 'boss-sourcing', 'references', 'runtime-contract.md'), 'utf8')
  : '';
const bossSourceGreetSkill = hasSkillWorkspace
  ? fs.readFileSync(path.join(skillRoot, 'boss-source-greet', 'SKILL.md'), 'utf8')
  : '';
const bossSourceBrowserStates = hasSkillWorkspace
  ? fs.readFileSync(path.join(skillRoot, 'boss-source-greet', 'references', 'browser-states.md'), 'utf8')
  : '';
const bossChatFollowupSkill = hasSkillWorkspace
  ? fs.readFileSync(path.join(skillRoot, 'boss-chat-followup', 'SKILL.md'), 'utf8')
  : '';
const bossResumeIngestSkill = hasSkillWorkspace
  ? fs.readFileSync(path.join(skillRoot, 'boss-resume-ingest', 'SKILL.md'), 'utf8')
  : '';

skillTest('boss-sourcing router documents split execution skills', () => {
...
skillTest('shared runtime contract documents runtime placeholders instead of machine-specific paths', () => {
...
skillTest('shared runtime contract forbids repo introspection and documents file-backed bootstrap writes', () => {
...
skillTest('shared runtime contract forbids recursive reference rediscovery and requires file-backed terminal callbacks', () => {
...
skillTest('boss-source-greet skill documents source callback identifier requirements', () => {
...
skillTest('boss-source-greet references document direct-render detail anchors and forbid utility-icon recovery', () => {
...
skillTest('boss-chat-followup skill documents attachment-state decisions', () => {
...
skillTest('boss-chat-followup skill requires draining unread queue before completion', () => {
...
skillTest('boss-resume-ingest skill documents runtime placeholders instead of machine-specific paths', () => {
...
skillTest('boss-resume-ingest skill forbids startup rediscovery scans when handoff context exists', () => {
